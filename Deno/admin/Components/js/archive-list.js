...
async function restoreDocument(docId) {
    try {
        ...
        if (card) {
            ...
            isCompiled = card.classList.contains('compiled') || card.classList.contains('compilation');
        }
        
        // Use the custom confirmation dialog if available
        if (window.documentArchive && typeof window.documentArchive.showRestoreConfirmation === 'function') {
            window.documentArchive.showRestoreConfirmation(docId, isCompiled, docTitle);
            return;
        }
        
        // Fall back to direct restore if the custom dialog isn't available
        // Show loading state
        updateLoadingState(true);

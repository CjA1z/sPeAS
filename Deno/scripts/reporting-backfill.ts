...
async function calculate(connection: any): Promise<Audit> {
  ...
  const hasAuthorCounters = await tableExists(connection, "author_visits_counter");
  const repository = hasDocumentVisits ? await connection.queryObject(`
    ...
    WHERE date < $1::date
  `, [cutoffDate]) : { rows: [{ rows: 0, ambiguous: 0, skipped: 0, invalid: 0 }] };
  const page = hasPageCounters
    ? await connection.queryObject("SELECT COUNT(*)::BIGINT AS rows FROM page_visits_counter WHERE date < $1::date", [cutoffDate])
    : { rows: [{ rows: 0 }] };
  const author = hasAuthorCounters
    ? await connection.queryObject("SELECT COUNT(*)::BIGINT AS rows FROM author_visits_counter WHERE date < $1::date", [cutoffDate])
    : { rows: [{ rows: 0 }] };
  const historyParts: string[] = [];
  const hasDocumentHistory = await connection.queryObject("SELECT to_regclass('public.user_document_history') IS NOT NULL AS exists");
  const hasCompiledHistory = await connection.queryObject("SELECT to_regclass('public.user_compiled_document_history') IS NOT NULL AS exists");
  if (hasDocumentHistory.rows[0]?.exists) historyParts.push("SELECT COUNT(*)::BIGINT AS rows FROM user_document_history h JOIN users u ON u.id::text = h.user_id::text WHERE LOWER(u.role) = 'user' AND h.accessed_at < $1");
  if (hasCompiledHistory.rows[0]?.exists) historyParts.push("SELECT COUNT(*)::BIGINT AS rows FROM user_compiled_document_history h JOIN users u ON u.id::text = h.user_id::text WHERE LOWER(u.role) = 'user' AND h.accessed_at < $1");
  let historyRows = 0;
...
async function run(): Promise<void> {
  const result = await withTransaction(async (connection) => {
    await connection.queryArray("SELECT pg_advisory_xact_lock(hashtext('peas-reporting-v2-backfill'))");
    const marker = await connection.queryObject("SELECT version FROM operational_analytics_backfills WHERE version = 'repository-activity-v2'");
    ...
    const hasLegacyDocumentVisits = await tableExists(connection, "document_visits");

    if (await tableExists(connection, "document_visits")) await connection.queryObject(`
      ...
      GROUP BY dv.date, dv.doc_id, dv.visitor_type
    ...
    if (await tableExists(connection, "page_visits_counter")) await connection.queryObject(`
      ...
      GROUP BY pvc.date, pvc.page_path, pvc.visitor_type
      ON CONFLICT (grain, bucket_start, page_key, audience)
      DO UPDATE SET visit_count = page_activity_rollups.visit_count + EXCLUDED.visit_count
    `, [cutoffDate]);

    if (await tableExists(connection, "author_visits_counter")) await connection.queryObject(`
      ...
      GROUP BY avc.date, a.id, avc.visitor_type
      ON CONFLICT (grain, bucket_start, author_id, audience)
      DO UPDATE SET visit_count = author_activity_rollups.visit_count + EXCLUDED.visit_count
    `, [cutoffDate]);
    ...
    const documentHistoryTables = await connection.queryObject("SELECT to_regclass('public.user_document_history') IS NOT NULL AS exists");
    const compiledHistoryTables = await connection.queryObject("SELECT to_regclass('public.user_compiled_document_history') IS NOT NULL AS exists");
    ...
    `, [cutoff, audit.ambiguousRepositoryRows, audit.skippedRepositoryRows, audit.skippedInvalidRows, JSON.stringify({ currentDayExcluded: true, legacyGranularity: "day" })]);
    await connection.queryObject(`
  ...
  console.log(JSON.stringify({ apply, cutoff: cutoff.toISOString(), ...result }, null, 2));
}

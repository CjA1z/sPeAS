...
await withTransaction(async (db) => {
  await db.queryArray(`
    ...
    TRUNCATE repository_activity_rollups, page_activity_rollups, author_activity_rollups;
    DELETE FROM operational_analytics_backfills WHERE version = 'repository-activity-v2';
    UPDATE operational_analytics_state SET writes_enabled = FALSE, reads_enabled = FALSE, live_started_at = NULL, last_backfill_version = NULL WHERE state_id = TRUE;
    ...
    INSERT INTO page_visits_counter VALUES ('/index.html/', (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Manila')::date - 2, 'guest', 6);

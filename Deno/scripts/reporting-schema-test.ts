...
const required = await client.queryObject<{ table_name: string }>(`
  ...
  WHERE table_schema = 'public' AND table_name IN
    ('repository_activity_rollups', 'page_activity_rollups', 'author_activity_rollups', 'operational_analytics_backfills', 'operational_analytics_state')
  ORDER BY table_name
`);
assertEquals(required.rows.length, 5);

const indexes = await client.queryObject<{ indexdef: string }>(`
  SELECT indexdef FROM pg_indexes
  WHERE schemaname = 'public' AND tablename IN ('repository_activity_rollups', 'page_activity_rollups', 'author_activity_rollups')
`);
...
const stateColumns = await client.queryObject<{ column_name: string }>(`
  ...
  WHERE table_schema = 'public' AND table_name = 'operational_analytics_state'
    AND column_name IN ('writes_enabled', 'reads_enabled')
`);
assertEquals(new Set(stateColumns.rows.map((row) => row.column_name)), new Set(["writes_enabled", "reads_enabled"]));

...
export function OperationalReportsPage() {
  ...
  return <main className="peas-admin-island peas-reports-page"><PeasToaster /><AdminPageHeader eyebrow="Operational visibility" title="Operational Reports" description="Review repository inventory, activity, archive status, and category distribution using canonical PeAS metrics." actions={<div className="peas-report-actions"><Select value={range} onValueChange={(value) => changeRange(value as ReportRange)}><SelectTrigger aria-label="Report time range" className="peas-report-range"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(RANGE_LABELS).map(([value, label]) => <SelectItem value={value} key={value}>{label}</SelectItem>)}</SelectContent></Select><Button variant="outline" disabled={loading || refreshing} onClick={() => void load(true)}><RefreshCw aria-hidden="true" /> {refreshing ? "Refreshing…" : "Refresh" }</Button><Button variant="outline" disabled={!stats || exporting.csv} aria-busy={exporting.csv} onClick={() => void exportReport("csv")}><Download aria-hidden="true" /> {exporting.csv ? "Preparing…" : "CSV"}</Button><Button disabled={!stats || exporting.pdf} aria-busy={exporting.pdf} onClick={() => void exportReport("pdf")}><Download aria-hidden="true" /> {exporting.pdf ? "Preparing…" : "PDF"}</Button></div>} />
    {error && !stats ? <PeasErrorState title="Unable to load reports" message={error} onRetry={() => void load(false)} /> : stats ? <>
      ...
      <section className="peas-report-kpis" aria-label={`${stats.meta.range.label} activity metrics`}><ReportKpi label="Uploaded entries" value={stats.activity.uploadedEntries} help={stats.metricDefinitions.uploaded_entries} /><ReportKpi label="Repository views" value={stats.activity.repositoryViews} help={stats.metricDefinitions.repository_views} /><ReportKpi label="Downloads" value={stats.activity.repositoryDownloads} help={stats.metricDefinitions.repository_downloads} /><ReportKpi label="Active registered readers" value={stats.activity.activeRegisteredUsers} help={stats.metricDefinitions.active_registered_users} /><ReportKpi label="Home visits" value={stats.activity.homeVisits.total} help={`${stats.metricDefinitions.home_visits} ${stats.activity.homeVisits.guest} guest + ${stats.activity.homeVisits.registered} registered-user visits.`} /><ReportKpi label="Guest repository views" value={stats.activity.guestViews} help={stats.metricDefinitions.guest_views} /><ReportKpi label="Registered repository views" value={stats.activity.registeredViews} help={stats.metricDefinitions.registered_views} /><ReportKpi label="Approved-request downloads" value={stats.activity.approvedRequestDownloads} help={stats.metricDefinitions.approved_request_downloads} /></section>
      <section className="peas-reports-grid"><SeriesPanel title="Uploads over time" rows={stats.series.uploads.map((row) => ({ bucket: row.bucket, primary: row.count }))} primaryLabel="Uploads" /><SeriesPanel title="Views and downloads" rows={stats.series.repositoryActivity.map((row) => ({ bucket: row.bucket, primary: row.views, secondary: row.downloads }))} primaryLabel="Views" secondaryLabel="Downloads" /><SeriesPanel title="Home traffic" rows={stats.series.homeVisits.map((row) => ({ bucket: row.bucket, primary: row.guest, secondary: row.registered }))} primaryLabel="Guest" secondaryLabel="Registered user" /></section>
      <section className="peas-reports-grid"><RankingPanel title="Most viewed catalog entries" rows={stats.rankings.mostViewedEntries.map((row) => ({ label: row.title, detail: `${row.category} · ${row.views} views`, value: row.views, href: row.href }))} /><RankingPanel title="Most downloaded catalog entries" rows={stats.rankings.mostDownloadedEntries.map((row) => ({ label: row.title, detail: `${row.category} · ${row.downloads} downloads`, value: row.downloads, href: row.href }))} /><RankingPanel title="Most visited authors" rows={stats.rankings.mostVisitedAuthors.map((row) => ({ label: row.name, detail: "Profile visits", value: row.visits, href: row.href }))} /><RankingPanel title="Trending topics" rows={stats.rankings.trendingTopics.map((row) => ({ label: row.name, detail: `${row.entryCount} works · ${row.views} views`, value: row.views, href: row.href }))} /></section>
      <section className="peas-reports-grid"><CategoryPanel rows={stats.distributions.documentTypes} /><RequestStatusPanel rows={stats.distributions.requestStatuses} /><ReaderSummary stats={stats} /></section>
...
function RequestStatusPanel({ rows }: { rows: Array<{ status: string; count: number }> }) { return <article className="peas-report-panel"><header><h2>Document requests by status</h2></header>{rows.length ? <dl className="peas-report-definition-list">{rows.map((row) => <div key={row.status}><dt>{row.status}</dt><dd>{row.count.toLocaleString()}</dd></div>)}</dl> : <PeasEmptyState title="No requests in this period" description="Request status counts will appear after access requests are submitted." />}</article>; }
...
function CoverageNotice({ coverage }: { coverage?: ReportStats["meta"]["coverage"] }) { const warnings = coverage ? [["Repository", coverage.repository?.warning], ["Home", coverage.home?.warning], ["Authors", coverage.authors?.warning]].filter((item): item is [string, string] => Boolean(item[1])) : []; return warnings.length ? <aside className="peas-report-coverage-warning" role="status"><strong>Historical coverage note</strong>{warnings.map(([label, warning]) => <span key={label}><b>{label}:</b> {warning}</span>)}</aside> : null; }

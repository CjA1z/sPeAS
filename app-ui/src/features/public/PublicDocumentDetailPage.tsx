...
import { BookCheck, BookOpen, CalendarDays, Check, Download, FileText, LockKeyhole, UserRound, X } from "lucide-react";
...
export function PublicDocumentDetailPage() {
  ...
  return <PublicPageShell mainClassName="peas-document-detail-page">{detail ? <DocumentContent id={id} detail={detail} authenticated={Boolean(session?.authenticated)} userId={session?.user?.id == null ? undefined : String(session.user.id)} onRequest={() => setRequestOpen(true)} /> : <p>Loading document details…</p>}{requestOpen && detail ? <AccessRequestDialog id={id} title={titleOf(detail.record)} onClose={() => setRequestOpen(false)} /> : null}</PublicPageShell>;
}

...
import type { NewsAuthorReference, NewsBodyFormat } from "../../lib/api/news";
import { NewsAuthorReferenceTag } from "./NewsArticleReferences";

export function NewsArticleBody(
  { body, format = "plain", authors = [] }: {
    body: string;
  ...
  return <>{parseMarkdownBlocks(body, authors)}</>;
}

function parseMarkdownBlocks(markdown: string, authors: NewsAuthorReference[]): ReactNode[] {
